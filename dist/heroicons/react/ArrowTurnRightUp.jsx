...
const ArrowTurnRightUp = forwardRef(({
  size = 24,
  color = 'currentColor',
  primaryColor,
  secondaryColor,
  ...
  return (
    <>
      ...
      <svg
        ...
        className={`animated-heroicon animated-heroicon-arrow-turn-right-up ${className}`}
        style={cssVars}
        role="img"
        aria-label={label}
        {...props}
      >
        <title>{label}</title>
        <path strokeLinecap="round" strokeLinejoin="round" d="m11.99 7.5 3.75-3.75m0 0 3.75 3.75m-3.75-3.75v16.499H4.49" className="ah-primary ah-anim-nudge ah-delay-0" style={{ '--ah-tx': '2px', '--ah-ty': '-2px' }} />
      </svg>
    </>
  );
});
