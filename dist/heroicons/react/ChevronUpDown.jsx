...
const ChevronUpDown = forwardRef(({
  size = 24,
  color = 'currentColor',
  primaryColor,
  secondaryColor,
  ...
  return (
    <>
      ...
      <svg
        ...
        className={`animated-heroicon animated-heroicon-chevron-up-down ${className}`}
        style={cssVars}
        role="img"
        aria-label={label}
        {...props}
      >
        <title>{label}</title>
        <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 15 12 18.75 15.75 15m-7.5-6L12 5.25 15.75 9" className="ah-primary ah-anim-nudge ah-delay-0" style={{ '--ah-tx': '0px', '--ah-ty': '2px' }} />
      </svg>
    </>
  );
});
