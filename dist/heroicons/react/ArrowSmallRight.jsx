...
const ArrowSmallRight = forwardRef(({
  size = 24,
  color = 'currentColor',
  primaryColor,
  secondaryColor,
  ...
  return (
    <>
      ...
      <svg
        ...
        className={`animated-heroicon animated-heroicon-arrow-small-right ${className}`}
        style={cssVars}
        role="img"
        aria-label={label}
        {...props}
      >
        <title>{label}</title>
        <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12h15m0 0-6.75-6.75M19.5 12l-6.75 6.75" className="ah-primary ah-anim-nudge ah-delay-0" style={{ '--ah-tx': '2px', '--ah-ty': '0px' }} />
      </svg>
    </>
  );
});
