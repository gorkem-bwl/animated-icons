...
const ArrowLongLeft = forwardRef(({
  size = 24,
  color = 'currentColor',
  primaryColor,
  secondaryColor,
  ...
  return (
    <>
      ...
      <svg
        ...
        className={`animated-heroicon animated-heroicon-arrow-long-left ${className}`}
        style={cssVars}
        role="img"
        aria-label={label}
        {...props}
      >
        <title>{label}</title>
        <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 15.75 3 12m0 0 3.75-3.75M3 12h18" className="ah-primary ah-anim-nudge ah-delay-0" style={{ '--ah-tx': '-2px', '--ah-ty': '0px' }} />
      </svg>
    </>
  );
});
