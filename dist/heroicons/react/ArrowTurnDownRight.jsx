...
const ArrowTurnDownRight = forwardRef(({
  size = 24,
  color = 'currentColor',
  primaryColor,
  secondaryColor,
  ...
  return (
    <>
      ...
      <svg
        ...
        className={`animated-heroicon animated-heroicon-arrow-turn-down-right ${className}`}
        style={cssVars}
        role="img"
        aria-label={label}
        {...props}
      >
        <title>{label}</title>
        <path strokeLinecap="round" strokeLinejoin="round" d="m16.49 12 3.75 3.75m0 0-3.75 3.75m3.75-3.75H3.74V4.499" className="ah-primary ah-anim-nudge ah-delay-0" style={{ '--ah-tx': '2px', '--ah-ty': '2px' }} />
      </svg>
    </>
  );
});
