...
const ArrowTurnLeftDown = forwardRef(({
  size = 24,
  color = 'currentColor',
  primaryColor,
  secondaryColor,
  ...
  return (
    <>
      ...
      <svg
        ...
        className={`animated-heroicon animated-heroicon-arrow-turn-left-down ${className}`}
        style={cssVars}
        role="img"
        aria-label={label}
        {...props}
      >
        <title>{label}</title>
        <path strokeLinecap="round" strokeLinejoin="round" d="m11.99 16.5-3.75 3.75m0 0L4.49 16.5m3.75 3.75V3.75h11.25" className="ah-primary ah-anim-nudge ah-delay-0" style={{ '--ah-tx': '-2px', '--ah-ty': '2px' }} />
      </svg>
    </>
  );
});
